...
function updateClock(type) {
    ...
    if (type === 'digital') {
        ...
        pendulumElement.style.display = 'none';  // Hide pendulum for digital clock
    } else if (type === 'analog') {
        ...
        // Show pendulum if analog clock is selected
        if (document.getElementById('pendulum').style.display === 'block') {
            pendulumElement.style.display = 'block';
        }
    }
}
...
clockTypeSelect.addEventListener('change', () => {
    ...
    // Show or hide pendulum option based on clock type
    if (selectedType === 'analog') {
        pendulumElement.style.display = 'block';  // Show pendulum for analog clock
    } else {
        pendulumElement.style.display = 'none';  // Hide pendulum for digital clock
    }
});

// Event listener for "Step 2" button click
step2Button.addEventListener('click', () => {
    window.location.href = 'step3.html'; // Change 'step2.html' to the name of your target page
});
